...
function getPlayerGuess(message = `guess the evil AI's number! 😈🤖 (between ${USER_NUMBER_MIN_LIMIT} and ${USER_NUMBER_MAX_LIMIT}.)`) {
    ...
    if (isNaN(userNumber)) {
        ...
        alert(`Please enter a number between ${USER_NUMBER_MIN_LIMIT} and ${USER_NUMBER_MAX_LIMIT}`)
        return getPlayerGuess();
    }
...
function game() {
    ...
    do {
        ...
        if (remainingAttempts <= 0) {
            alert(`Better luck next time! You didn't guess the number correctly. The correct answer was ${randomNumber}\n\nRefresh the page to try again.`)
            break;
        }
        
        if (checkedGuess === "correct") {
            alert(`You Win! 👑 The correct answer was ${randomNumber}\n\nYou managed to guess the number in ${ATTEMPTS_NUMBER} attemps. ${generateAttemptsMessage(ATTEMPTS_NUMBER)}\n\nRefresh the page to play again! 👾`);
            break;
        } else if (checkedGuess === "high") {
            alert(`The number you've chosen is too high!\n\n${remainingAttempts} attempts remaining...`)
        } else {
            alert(`The number you've chosen is too low!\n\n${remainingAttempts} attempts remaining...`)
        }
